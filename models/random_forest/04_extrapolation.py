"""
Random Forest — Extrapolation to Unseen Region (Gapeau Basin)

Steps:
  1. Load trained RF model (trained on Argens)
  2. Load feature rasters for Gapeau basin
  3. Predict flood probability per pixel (block-wise)
  4. Handle nodata pixels
  5. Write prediction raster
...
OUTPUT_PATH = OUTPUT_DIR / "Gapeau_RF_4dtsts_extrapolated.tif"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# ── Feature rasters (must match training order) ─────────
...
def extrapolate(model, feature_paths, output_path):
    ...
    Handles nodata pixels gracefully.
    ...
    feature_paths : list
        Ordered list of feature raster paths.
    output_path : Path
        Output prediction raster path.
    """
    ...
    with rasterio.open(output_path, "w", **meta) as dst:
        for block_idx, (_, window) in enumerate(srcs[0].block_windows(1)):
            # Read each feature band for this window
            bands = [src.read(1, window=window) for src in srcs]
            arr = np.stack(bands, axis=0)  # (n_feats, H, W)
            n_feats, H, W = arr.shape

            # Reshape to (n_pixels, n_feats)
    ...
    print(f"  ✅ Extrapolation raster written to: {output_path}")


def main():
    print("=" * 60)
    print("RANDOM FOREST — EXTRAPOLATION TO GAPEAU BASIN")
    print("=" * 60)
